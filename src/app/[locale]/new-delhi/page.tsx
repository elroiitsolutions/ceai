...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const fetched = await fetchAPI('/city-pages', { 
    locale, 
    filters: { slug: { $eq: 'new-delhi' } }
  });
  const data = fetched?.data?.[0];
  
  return {
    title: data?.cityName ? `${data.cityName} - Government Policies` : 'New Delhi - Government Policies',
    description: data?.subtitle || 'India\'s capital and political center with core preferential policy support and business environment.',
  };
}

export default async function NewDelhiPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  ...
  const subtitle = cityData?.subtitle || "India's capital and political center with core preferential policy support and business environment.";
  const cityName = cityData?.cityName || "New Delhi";
